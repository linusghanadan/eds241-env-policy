...
combined_results <- rbind(prop_test_results, t_test_results)

# Create the kable table
kable_table <- kable(combined_results, format = "html", caption = "Combined Results of Proportion and T-Tests") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)

# Print the table to view it in an R Markdown document or R Notebook
kable_table
...
progresa <- progresa %>% 
  arrange(hhid, year) %>% 
  group_by(hhid) %>% # WHY NOT GROUP BY YEAR AS WELL
...
progresa <- progresa[!is.na(progresa$treatment), ]
progresa <- progresa[!is.na(progresa$vani), ]
progresa <- progresa[!is.na(progresa$hhid), ]
progresa <- progresa[!is.na(progresa$year), ]

within_reg <- plm(vani ~ treatment, index = c("hhid", "year"), model = c("within"), effect = c("twoways"), data = progresa)
