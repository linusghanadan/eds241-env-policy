...
model <- lm(vani_mean ~ treatment_dummy + post_treatment_time_dummy + post_treatment_time_dummy*treatment_dummy, data = ols_df)

# Report OLS Model results
summary(model)
...
## OLS regression
model_full <- lm(vani ~ treatment_dummy + post_treatment_time_dummy + interaction_dummy, data = progresa_full)

# Present Regressions in Table
summary_reg <- summary(model_full)
summary_reg$coefficients %>% 
  kbl(caption = "DiD estimation") %>%  # Generate table
  kable_classic(full_width = FALSE)
