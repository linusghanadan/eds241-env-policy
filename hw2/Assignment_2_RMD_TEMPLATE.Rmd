...
[Using the WLS estimator, we estimate the average effect on birth weight of maternal smoking to be -224.9 grams. Our standard error of 3.2 grams indicates that our 95% confidence interval for the effect size is between -231.2 grams and -218.6 grams. This estimated effect size applies to all newborns because we did not remove any newborns from our sample as we did when estimating effect size based on matching. Instead, we weighted each newborn based on how similar their covariates were to the average covariate values, meaning some newborns were weighted more than others but none were completely removed.]{style="color:navy;"}
